...
/*解説
2回以上購買しているユーザをリピートとみなしてリピート率を計算すればよい。
そのためには仮想テーブル内のrepeat_usersの中身を書き換える。
元のクエリではdistinct orders.user_idでuser_idのユニークリストを取得していたが、
ここではGROUP BYとCOUNTを組み合わせてuser_idごとのリピート回数を算出して、
havingを使うことでリピート回数が2回以上のユーザのみを残すようにしている。
*/
